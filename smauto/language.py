...
from smauto._processors import get_obj_processors
from smauto._builtins import ENTITY_BUILTINS  # noqa: F401 — re-exported for backward compat
    ...
    for action in actions or []:
        if action.__class__.__name__ == "GroupSetAction":
            ...
            for entity in group.members:
                ...
                # Synthesise a plain action object — templates only need .attribute and .value
                val = action.value
                new_action = python_types.SimpleNamespace(
...
def get_metamodel(debug: bool = False, global_repo: bool = False):
    ...
    metamodel.register_obj_processors(get_obj_processors())
