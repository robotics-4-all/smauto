...
class ValueGeneratorProperties:
    ...
    class Uniform:
      min: float
      max: float # this is time (seconds)
...
if __name__ == '__main__':
    ...
    temperature_component = ValueGeneratorComponent(
    ...
    # Linear example
    pressure_properties = ValueGeneratorLinear(start = 27, increase = 0.1)
    pressure_noise = Noise(
      _type = NoiseType.Gaussian,
      properties = NoiseGaussian(mu = 0, sigma = 0.3)
    )
    pressure_component = ValueGeneratorComponent(
    ...
    # Gaussian example
    humidity_properties = ValueGeneratorGaussian(value = 27, max_value = 32, sigma = 2)
    humidity_noise = Noise(
      _type = NoiseType.Gaussian,
      properties = NoiseGaussian(mu = 0, sigma = 0.3)
    )
    humidity_component = ValueGeneratorComponent(
    ...
    generator = ValueGenerator(namespace = "smauto", name = "bme", hz = 2, components = components, connection = connection)
