...
class MotionDetectorNode(Node):
    ...
    def init_gen_components(self):
        components = []
        detected_properties = ValueGeneratorProperties.Replay(
        ...
        posX_properties = ValueGeneratorProperties.Saw(min=0, max=10, step=1)
        ...
        posY_properties = ValueGeneratorProperties.Gaussian(
            value=0,
            max_value=10,
            sigma=1,
        )
        _gen_type = ValueGeneratorType.Gaussian
        posY_noise = Noise(_type=NoiseType.Uniform, properties=NoiseUniform(0, 1))
