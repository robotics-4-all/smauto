...
class WeatherStationNode(Node):
    def __init__(self, *args, **kwargs):
        self.pub_freq = 20
        self.topic = 'home.weather_station'
    ...
    def init_gen_components(self):
        ...
        pressure_properties = ValueGeneratorProperties.Constant(
            value=0.5
        )
        _gen_type = ValueGeneratorType.Constant
        pressure_noise = Noise(
            _type=NoiseType.Uniform,
            properties=NoiseUniform(0, 0.2)
        )
...
"""
    ...
    def gen_data(self):
        ...
        msg.temperature = random.gauss(, 5)
        if not hasattr(self, "_humidity"):
            val = 
        else:
            val = self._humidity + 0.2
        if val > :
            val = 
        self._humidity = val
        msg.humidity = val
        msg.pressure = attr.generator.value
        return msg
