...
- **Condition evaluation via `eval()`**: Conditions are built as Python expression strings at parse time, then evaluated at runtime with `eval()` and entity + automations dict context
- **Automation status conditions**: `automation_name.status == STATUS` in `when` block enables inter-automation coordination (statuses: IDLE/RUNNING/SUCCESS/FAILED/FINISHED/TERMINATED)
- **Automation grammar uses ECA (Event-Condition-Action) syntax**: `Automation name when condition then actions config properties triggers ... terminates ... end`
